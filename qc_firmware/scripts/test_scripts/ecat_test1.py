...
def read_temp (addr):
    try:
        ...
        print ('Temp sensor addr ' + str(addr) + ' reads raw value: ' + i2c_raw + ' which is temp: ' + format(temp, '0.1f') + ' C')
...
def read_volt (addr):
    try:
        ...
        print ('Voltage sensor addr ' + str(addr) + ' reads raw value: ' + i2c_raw + ' which is voltage: ' + format(volts, '0.1f') + ' V')
    except Exception as e:
        print ('Sensor ' + str(addr) + ' not readable')
        raw_word = char_to_raw('adde', 4)
    return raw_word
...
def read_curr (addr, resistor):
    try:
        ...
        print ('Voltage sensor addr ' + str(addr) + ' reads raw value: ' + i2c_raw + ' which is current: ' + format(curr, '0.2f') + ' A')
    except Exception as e:
        print ('Sensor ' + str(addr) + ' not readable')
        raw_word = char_to_raw('adde', 4)
    return raw_word
