...
async def run_test(ws):
    async for message in ws:
        try:
            ...
            lg.critical(f"Invalid JSON Paylod! Got {message}")
        ...
        with test_registry[test_info['test_type']](**test_params) as t: # type: ignore
            lg.info(f"Running Test {test_info['test_type']}") # type: ignore
            r = t.run_test()
            lg.info(f"Test Result: {r}")
            await send_test_result(ws, test_info['test_type'], r) # type: ignore
