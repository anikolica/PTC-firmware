...
v1.2 & 2026-04-01 & A. Nikolica & Clarifications in 4.2.2 and 4.4.5 for timing tests \\
v1.3 & 2026-05-28 & C. Zuschmidt & Modified testing based on first runthrough of procedures \\
\hline
...
\label{section:timing_card}
    ...
    \begin{enumerate}
        ...
        \item Connect to the serial UART with \texttt{minicom -D /dev/ttyUSB[x]} where x is probably 0 if there are no other devices connected -- you can use 
    ...
    \begin{enumerate}
        ...
        \item Next, prepare the bootable SD card for the DUT (these steps are listed on the github page for the PTC firmware, but are repeated here for clarity):
            \begin{enumerate}
                ...
                \item Ensure that the networking-stack package is enabled for later ethernet testing
            ...
            \end{enumerate}
        \item Next, insert the SD card prepared in the previous step into the PTC's SD card slot. Ensure the SD card clicks firmly into place with little force. Use tweezers if necessary. If the SD card is not engaging, it may be inserted upside down. Do not force the card.
    ...
    \begin{enumerate}
        \item \textbf{Power up}
            \begin{enumerate}
                ...
                \item Voltage check: measure with the DVM at the test point on the PTC labeled VP12\_LOCAL and ensure it is 12V. \textcolor{blue}{\textbf{Acceptance criteria: 12.00--12.35V}}.
...
\label{section:dcdc_check}
    ...
    \begin{enumerate}
        \item \textbf{Voltage Check} 
            \begin{enumerate}
                \item\textcolor{red}{Ensure the Enclustra SoM is NOT installed on PTC board at this point.}
...
\label{section:load_tests}
    \begin{enumerate}
        ...
        \item \textbf{Enable PWM driver} While ssh'd into the QC board, type \texttt{poke 0x8002004c 0x316} followed by the value \texttt{0x216}. This sets the PWM driver to 2MHz and 25\% duty cycle. It can be tested by measuring the PWM probe point. 
        ...
        \item \textbf{Repeat with other load values} The values of \texttt{0x226, 0x236, 0x23e} will test 50\%, 75\%, and approximately 100\% load on the converter. Do not leave tests over 50\% running for more than a few minutes. Confirm the coresponding current draws are 3A, 4.5A, and 6A. \textcolor{blue}{\textbf{Acceptance criteria: $3A\pm100mA$, $4.5A\pm100mA$, $6A\pm100mA$}}.
        \item \textbf{Measure voltage ripple} At the highest load, measure the test point RIPPLE\_0 with the scope set on 200mV scale, 200ns period, and AC coupling, and note the AC ripple with an RMS measurement. \textcolor{blue}{\textbf{Acceptance criteria: $\pm200mV$}}. An example is shown in Figure~\ref{dcdc_ripple}.
            \begin{figure}[h!]
                \centering             
                ...
                \label{dcdc_ripple}
            \end{figure}
        \item \textbf{Repeat with other converters} The addresses of \texttt{0x80020048, 0x80020044, 0x80020040, 0x8002003c, 0x80020038} correspond to converters 1 through 5. Repeat the test for all converters. Acceptance criteria are same as above.
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Sensor Extended Functionality}
            \begin{enumerate}
                ...
                \verb|qc_firmware\scripts\test_scripts\qc_board_tests\alert_assert.py| script. This will go through each sensor, program a threshold such that the ALERT pin is asserted, and readback the ALERT register over I2C. \textcolor{blue}{\textbf{Acceptance criteria: PASS/FAIL}}.
    ...
    \begin{enumerate}
        \item \textbf{Setup} The complete setup instructions are shown on the PTC github page README, and are repeated here for clarity:
            \begin{enumerate}
                ...
                \item The first time the link is set up, or if any configuration changes, copy the XMC\_ESC.xml file (the "ESI file") from \verb|ethercat/xmc4300_proj/xmc4300_proj/SSC/XMC_ESC.xml| to
...
\end{verbatim}
        \textcolor{blue}{\textbf{Acceptance criteria: PASS/FAIL}}.
            ...
            \item Timing lock on QC board (\texttt{\textcolor{red}{NOTE:}} Only for ch0; add steps for ch1-5.)
\begin{verbatim}
root@qc:~# poke 0x80020004 0x00000400
root@qc:~# poke 0x80020004 0x00000000
root@qc:~# peek 0x80020104
0x18000000 # endpoint out of reset, and showing 0x8 status (good)
\end{verbatim}
        \textcolor{blue}{\textbf{Acceptance criteria: PASS/FAIL}}.
        \end{enumerate}
        \item \textbf{TX Test}
...
\label{section:eth_tests}
    ...
    \begin{enumerate}
        \item \textbf{Ping Test}
            \begin{enumerate}
                \item Front panel LINK LED. \textcolor{blue}{\textbf{Acceptance criteria: PASS/FAIL}}.
                \item Ping. The DUT should come up as 192.168.200.12 on the network. Issue the \texttt{ping} commands from the Linux test machine and ensure the DUT responds. \textcolor{blue}{\textbf{Acceptance criteria: PASS/FAIL}}.
            \end{enumerate}
        \item \textbf{Throughput Test}. Using iPerf3, run the command \texttt{iperf3 -s} on the DUT through minicom, followed by \texttt{iperf3 -c 192.168.200.12} on the Linux test machine. Alternatively, You can ssh into the PTC as user root. 
