...
setup_dbt dunedaq-v3.1.1 # up to here needs to be done every time
...
\label{section:timing_card}
    ...
    \begin{enumerate}
        ...
        \item Connect to the serial UART with \texttt{minicom}. The serial port will show up as /dev/ttyUSB[x] where x is probably 0 (if there are no other devices connected). The settings should be: 19,200 baud, 8 bits, no parity bits, 1 stop bit, no hardware flow control.
        \item \texttt{help} shows a list of commands. Confirm the serial port returns output.
        \item \texttt{write} followed by \texttt{0xc0a8c810} on the following line will set the IP address as 192.168.200.16.
